...
for i in social_structures:
    ...
    fname = f'heatmap_res{res}_{i}_beta={beta}_{run_length}_{ensemble_size}'
    sb.heatmap((Bstar_arr-Astar_arr)[::-1,:], xticklabels=np.round_(props[::], decimals=3), yticklabels=np.round_(qs[::], decimals=3), cbar_kws={'label':r'$n_{B}^{\ast}-n_{A}^{\ast}$'}, cmap="RdBu")
    plt.xlabel('p')
    plt.ylabel(r'$q$')
