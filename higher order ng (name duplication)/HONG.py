...
class HigherOrderNamingGame(xgi.Hypergraph):
    ...
    def interact_and_advance(self, edge, verbose=False):
        ...
        if self.rule == 'Unanimous':
            if all([broadcast in self.get_attr(i, 'vocab') for i in edge]):
                ...
                for j in edge:
                    if not self.get_attr(j, 'committed'):
    ...
    def run(self, edges, runlength, verbose=False):
        ...
        vocab_counts['AB'][0] = self.count_by_attr('vocab', ['A', 'B'], False)+self.count_by_attr('vocab', ['B', 'A'], False) #this may be the cause of a problem later if we initialise with some mixed vocabularies
